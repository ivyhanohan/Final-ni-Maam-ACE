...
import { Label } from "@/components/ui/label";
import { useToast } from "@/hooks/use-toast";
import { ArrowLeft, Wallet, Image as ImageIcon } from 'lucide-react';
...
const PaymentForm: React.FC<PaymentFormProps> = ({ onPaymentComplete, onCancel }) => {
  ...
  const schoolGcashNumber = "09171234567"; // <- School's GCash number (fixed)
  ...
  const handlePaymentSubmit = (e: React.FormEvent) => {
    ...
    if (!gcashName || !screenshotFile) {
      toast({
        title: "Missing Information",
        description: "Please enter your GCash account name and upload your payment screenshot.",
        variant: "destructive",
      });
      setIsProcessing(false);
      return;
    ...
    setTimeout(() => {
      const paymentDetails = {
        gcashName,
        gcashNumber: schoolGcashNumber.replace(/\d(?=\d{4})/g, "*"),
        screenshotFileName: screenshotFile.name,
      };

      onPaymentComplete("gcash", paymentDetails);
      setIsProcessing(false);

      toast({
        title: "Payment Successful",
        description: "Your GCash payment and screenshot have been submitted successfully.",
      });
    }, 2000);
  };
  ...
  return (
    <Card className="shadow-lg max-w-2xl mx-auto">
      <CardHeader>
        <CardTitle className="text-2xl">Payment Information</CardTitle>
        <CardDescription>
          Please send your GCash payment to the school's GCash number and upload your payment proof.
        </CardDescription>
      </CardHeader>
      ...
      <CardContent>
        <form onSubmit={handlePaymentSubmit}>
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div>
                <Label htmlFor="gcashName">Your GCash Account Name</Label>
                <Input
                  id="gcashName"
              ...
              <div>
                <Label htmlFor="gcashNumber">School's GCash Number</Label>
                <Input
                  id="gcashNumber"
                  value={schoolGcashNumber}
                  readOnly
