...
//               { id: 4, name: "2x2 ID Picture", status: "Pending", date: new Date().toISOString().split('T')[0] },
//             ]
//           };
...
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";
import { Clock, Calendar, CheckCircle, AlertCircle, Home, LogOut, XCircle } from 'lucide-react';
import { useToast } from "@/hooks/use-toast";
...
const Dashboard = () => {
  ...
  // New function: override document status depending on application status
  const getDocumentStatus = (docStatus: string) => {
    const appStatus = applicationStatus?.status.toLowerCase();
    if (appStatus === 'approved') return 'Verified';
    if (appStatus === 'rejected') return 'Rejected';
    return docStatus; // otherwise use the original
  };
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <Table>
                  ...
                  <TableBody>
                    {applicationStatus.documents.map((doc, index) => {
                      const finalStatus = getDocumentStatus(doc.status);
                      return (
                        <TableRow key={index}>
                          <TableCell>{doc.name}</TableCell>
                          <TableCell>
